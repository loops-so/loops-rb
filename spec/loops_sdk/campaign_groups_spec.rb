...
RSpec.describe LoopsSdk::CampaignGroups do
  ...
  describe ".create" do
    it "makes a POST request to create a campaign group" do
      expect(connection).to receive(:send).with(:post) do |&block|
        req = double("req")
        expect(req).to receive(:url).with("v1/campaign-groups")
        expect(req).to receive(:headers=).with(default_headers)
        expect(req).to receive(:params=).with({})
        expect(req).to receive(:body=).with({ name: "Newsletters", description: "Monthly" }.to_json)
        block.call(req)
        response
      end
